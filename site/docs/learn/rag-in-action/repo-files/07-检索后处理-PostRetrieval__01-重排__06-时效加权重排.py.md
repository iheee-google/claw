# 文件：07-检索后处理-PostRetrieval/01-重排/06-时效加权重排.py

- 相对路径：`07-检索后处理-PostRetrieval/01-重排/06-时效加权重排.py`
- 大小：`8518` bytes
- 原文件：[07-检索后处理-PostRetrieval/01-重排/06-时效加权重排.py](/learn/rag-in-action/repo-raw/rag-in-action/07-检索后处理-PostRetrieval/01-重排/06-时效加权重排.py)

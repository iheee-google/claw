...
2. **长任务分段汇报**：超过 10 秒的操作，必须给中间状态，不允许静默等待。
3. **避免长阻塞**：优先短超时 + 分段轮询，避免单次阻塞太久。
4. **超时自动降级**：外部查询慢时先给部分结果，再补全。

### ⏱️ 防静默硬约束（本会话强化）

5. **首次回执时限**：收到用户请求后，3 秒内必须先回执。
6. **持续进度频率**：执行中若超过 8 秒未完成，每 15 秒至少汇报一次状态。
7. **挂起快速止损**：工具命令无输出挂起超过 20 秒，必须立即中断并改为降级方案/替代路径。
8. **禁止无声完成**：任务完成后必须立即回报结果，不得出现“做完但未回复”。

### 🛡️ 防空输出流程（必须执行）

9. **非空校验**：发送前必须确认回复正文非空；若正文为空，禁止发送空消息。
10. **占位回执兜底**：当内容尚未准备好时，先发“收到，处理中”，再补完整结果。
11. **并发防覆盖**：连续多条用户消息场景，优先回复最新消息，并在必要时补一句“上一条已处理”。

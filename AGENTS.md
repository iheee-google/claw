...
1. **挂起快速止损**：工具命令无输出挂起超过 20 秒，必须立即中断并改为降级方案/替代路径。
2. **禁止无声完成**：任务完成后必须立即回报结果，不得出现“做完但未回复”。
3. **非空校验**：发送前必须确认回复正文非空；若正文为空，禁止发送空消息。
4. **并发防覆盖**：连续多条用户消息场景，优先回复最新消息，并在必要时补一句“上一条已处理”。
5. **异常降级文案**：若工具异常/超时，必须返回明确说明与下一步，不允许留空。
